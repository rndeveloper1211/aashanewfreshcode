...
const ImpsNeftScreen = () => {
  ...
  const handleShare = useCallback(async (item: any) => {
    const rows = [
      ...
      `<tr><td style="padding:9px 14px;color:#6B7280;font-size:13px">${k}</td>` +
      `<td style="padding:9px 14px;font-weight:700;font-size:13px;text-align:right">${v}</td></tr>`
    ).join('');

    const html = `<html><head><meta name="viewport" content="width=device-width"/>
      <style>body{font-family:sans-serif;margin:0;background:#F9FAFB}h2{text-align:center;color:#1D4ED8;padding:20px 0;margin:0}table{width:100%;border-collapse:collapse}tr:nth-child(even){background:#F3F4F6}td{border-bottom:1px solid #E5E7EB}.footer{text-align:center;padding:20px;color:#9CA3AF;font-size:12px}</style>
      </head><body><h2>IMPS / NEFT Receipt</h2><table>${rows}</table>
      <div class="footer">Powered by ${APP_URLS.AppName}</div></body></html>`;

    try {
      const file = await RNHTMLtoPDF.convert({
        html,
        fileName:  `IMPS-${fv(IsDealer, item.trans_id, item.TransactionId) ?? Date.now()}`,
        directory: 'Documents',
      });
      await Share.open({ title: 'Share Receipt', url: `file://${file.filePath}` });
    } catch (err: any) {
